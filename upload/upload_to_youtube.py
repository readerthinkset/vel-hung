"""
YouTube Upload Script - VELOCITY HUNGARIAN
"""
import os
...
def generate_video_metadata(category, num_phrases, phrases=None):
    title = f"Hungarian Learning: {num_phrases} Essential {category} Phrases"
    lines = [f"Learn Hungarian with VELOCITY HUNGARIAN!", "", f"Category: {category}", "", f"Master Hungarian one phrase at a time! Today's {category} lesson:", ""]
    if phrases:
        for i, p in enumerate(phrases[:5], 0):
            lines.append(f"{i+1}. {p['english']}")
            lines.append(f"   {p.get('hungarian', '')}")
            lines.append(f"   [{p.get('transliteration', '')}]")
            lines.append("")
    lines.extend(["Tip: Repeat each phrase out loud 3 times!", "Like this video if you learned something new!", "Subscribe for daily Hungarian lessons!", "", f"#LearnHungarian #HungarianLessons #HungarianForBeginners #LanguageLearning", f"#Hungarian #Education #Tutorial #DailyHungarian #{category.replace(' ', '')}", f"#VELOCITYHUNGARIAN #HungarianPhrases #SpeakHungarian"])
    tags = [f"learn hungarian", f"hungarian lessons", f"hungarian for beginners", f"hungarian phrases", "language learning", f"speak hungarian", category.lower(), "education", f"daily hungarian", "velocity hungarian", f"hungarian learning"]
    return title, "\n".join(lines), tags

def upload_to_youtube(video_path, title, description, tags=None, category_id='22'):
    if tags is None: tags = ['education', 'language learning', '#Hungarian']
...
def main():
    ...
    try: upload_to_youtube(video_path=video_file, title=f"Learn Hungarian Daily", description=f"#shorts #learnhungarian #hungarian #language #education", tags=['#Hungarian', 'education', 'language learning'], category_id='22')
