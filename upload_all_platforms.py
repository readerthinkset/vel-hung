"""
VELOCITY HUNGARIAN - Unified Social Media Upload Script
"""
...
def get_latest_reel():
    ...
    return {"video_path": str(latest), "metadata": meta, "category": meta.get("category_english", "Hungarian Learning"), "phrases": meta.get("phrases", [])}


def generate_caption(phrases, category, platform="facebook"):
    base = [f"Learn Hungarian with VELOCITY HUNGARIAN!", "", f"Category: {category}", "", f"Master Hungarian one phrase at a time! Today's {category} lesson:", ""]
    for i, p in enumerate(phrases[:5], 0):
        base.append(f"{i+1}. {p['english']}")
        base.append(f"   {p.get('hungarian', '')}")
        base.append(f"   [{p.get('transliteration', '')}]")
        base.append("")
    base.extend(["Tip: Repeat each phrase out loud 3 times!", "Like this video if you learned something new!", "Comment your favorite phrase below!", f"Follow for daily Hungarian lessons!", ""])
    base.extend([f"#learnhungarian", f"#hungarianlessons", f"#hungarianforbeginners", "#languagelearning", f"#hungarianvocabulary", f"#velocityhungarian", f"#dailyhungarian", f"#hungarian", "#learnlanguages"])
    return "\n".join(base)
...
def upload_to_all_platforms(video_path, caption, category, phrases=None):
    ...
    print("\n" + "="*80)
    print(f"VELOCITY HUNGARIAN - MULTI-PLATFORM UPLOAD")
    print("="*80)
...
def main():
    print("\n" + "="*80)
    print(f"VELOCITY HUNGARIAN - AUTOMATED UPLOAD")
    print("="*80)
